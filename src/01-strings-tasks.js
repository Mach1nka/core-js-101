...
function getRectangleString(width, height) {
  ...
  const lastStr = `${corns[2]}${str1}${corns[3]}\n`;
  const firstStr = `${corns[0]}${str1}${corns[1]}\n`;
  const str = ' '.repeat(width - 2);
  const betwStr = `│${str}│\n`;
  const string = '';
  return string.concat(firstStr, betwStr.repeat(height - 2), lastStr);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].charCodeAt() === 32) {
      res += ' ';
    } else if (str[i].charCodeAt() === 33) {
      res += '!';
    } else if (str[i].charCodeAt() === 63) {
      res += '?';
    } else if ((str[i].charCodeAt() > 77 && str[i].charCodeAt() < 91)
    || (str[i].charCodeAt() > 109 && str[i].charCodeAt() < 123)) {
